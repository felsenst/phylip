...
/* Need new count function because existing count assumes steps is a bitset. */
...
node *root;
long outgrno, screenlines, col, treelines, leftedge, topedge, vmargin, hscroll, vscroll, scrollinc, screenwidth, farthest, whichtree, othertree;
...
/* RSGdebug: What is going on here? */
...
void new_move_fillin(node *p)
{
  ...
  printf("new_move_fillin: index = %d\n", p->index);
  ...
  for (i = 0; i < chars; i++)
  {
    ...
    if (wa) {
      ...
      do
      {
        if (i == 0)
        {
          printf("child statezero = ");
          for (ii=5; ii >= 0; ii--) {
            printf("%d", (q->back->statezero[j-1] & (1L << ii)) > 0);
          }
          puts("");
          printf("child stateone  = ");
          for (ii=5; ii >= 0; ii--) {
            printf("%d", (q->back->stateone[j-1] & (1L << ii)) > 0);
          }
          puts("");

        }

        if ((q->back->statezero[j-1]) & (1L << k)) {
          n0++;
        }
        if ((q->back->stateone[j-1]) & (1L << k)) {
          n1++;
        }
        q = q->next;
      } while (q != p);
      if (n1 > n0) {
        ...
        if (n0 > n1) {
          /*puts("n0 > n1");*/
          p->statezero[j-1] |= (1L << k);
        } else { /* n0, n1 equal */
          puts("n0 = n1 case now left out for test");
          /*p->statezero[j-1] |= (1L << k);
            p->stateone[j-1] |= (1L << k);*/
        }
      }
      ...
      /*xx work here */
      if ((n - n1) <= (n - n0)) {
        /* printf("steps[%d] value = %d\n", i, n - n1);*/
        steps_a[i] = ((n - n1) > 1 ? 0 : (n - n1));
      } else {
        /* printf("steps[%d] value = %d\n", i, n - n0);*/
        steps_a[i] = ((n - n0) > 1 ? 0 : (n - n0));
      }
      /*xx*//*printf("new_move_fillin: n = %d, n0 = %d, n1 = %d\n",n,n0,n1);*/

      /*xx*//*printf("new_move_fillin: steps_a[%d] = %d\n",i,steps_a[i]);*/
    } else {
  ...
  puts("");
  /*xx debug print stuff begin */
  printf("steps = ");
  for (ii=5; ii >= 0; ii--) {
    j = ii / bits + 1;
    k = ii % bits;
    printf("%d", (steps[j-1] & (1L << k)) > 0);
  }
  puts("");
  printf("p->statezero = ");
  for (ii=5; ii >= 0; ii--) {
    j = ii / bits + 1;
    k = ii % bits;
    printf("%d", (p->statezero[j-1] & (1L << k)) > 0);
  }
  puts("");
  printf("p->stateone  = ");
  for (ii=5; ii >= 0; ii--) {
    j = ii / bits + 1;
    k = ii % bits;
    printf("%d", (p->stateone[j-1] & (1L << k)) > 0);
  }
  puts("\n");
  /*xx debug print stuff end */
  puts("new_move_fillin ending ----");

} /* new_move_fillin */


void debugnode(node *p)
{
  node *q;
  long n = 0;
  long i, j, k;
  ...
  printf("left child statezero = ");
  for (ii=5; ii >= 0; ii--) {
    j = ii / bits + 1;
    k = ii % bits;
    printf("%d", (p->next->back->statezero[j-1] & (1L << k)) > 0);
  }
  puts("");
  printf("left child stateone  = ");
  for (ii=5; ii >= 0; ii--) {
    j = ii / bits + 1;
    k = ii % bits;
    printf("%d", (p->next->back->stateone[j-1] & (1L << k)) > 0);
  }
  puts("\n");

  printf("right child statezero = ");
  for (ii=5; ii >= 0; ii--) {
    j = ii / bits + 1;
    k = ii % bits;
    printf("%d", (p->next->next->back->statezero[j-1] & (1L << k)) > 0);
  }
  puts("");
  printf("right child stateone  = ");
  for (ii=5; ii >= 0; ii--) {
    j = ii / bits + 1;
    k = ii % bits;
    printf("%d", (p->next->next->back->stateone[j-1] & (1L << k)) > 0);
  }
  puts("\n");
} /* debugnode */
...
void mycount(long *stps, bitptr zeroanc, steptr numszero, steptr numsone)
{
...
void newcount(long *stps, bitptr zeroanc, steptr numszero, steptr numsone)
{
  /* counts the number of steps in a fork of the tree.
     The program spends much of its time in this PROCEDURE */
  /* used in mix & penny */
...
void move_postorder(node *p)
{
  ...
  /*  move_fillin(p);*/
  new_move_fillin(p); /*debugnode(p);*/

  /*  count(steps, zeroanc, numszero, numsone);*/
  newcount(steps_a, zeroanc, numszero, numsone);
}  /* move_postorder */
...
void evaluate(node *r)
{
  ...
  nextcompat = true;
  move_postorder(r);
  mycount(r->stateone, zeroanc, numszero, numsone);
  for (i = 0; i < words; i++)
  {
    zeroanc[i] = 0;
    oneanc[i] = fullset;
  ...
  move_postorder(r);
  mycount(r->statezero, zeroanc, numszero, numsone);
  for (i = 0; i < chars; i++)
  {
...
void move_hyptrav(node *r)
{
  ...
  if (bottom) {
    ...
    memcpy(zerobelow->bits_, treenode[r->back->index - 1]->statezero, words * sizeof(long));
    memcpy(onebelow->bits_, treenode[r->back->index - 1]->stateone, words * sizeof(long));
  }
  for (i = 0; i < words; i++) {
    s0 = r->statezero[i];
    s1 = r->stateone[i];
...
void arbitree(void)
{
  ...
  root = treenode[0];
  add2(treenode[0], treenode[1], treenode[spp], &root, restoring, wasleft, treenode);
  for (i = 3; i <= spp; i++)
                  ...
                  FILE *intree)
{
  ...
  {
    case bottom:
      gnutreenode(grbg, p, nodei, chars);
      treenode[nodei - 1] = *p;
      break;
    case nonbottom:
      gnutreenode(grbg, p, nodei, chars);
...
void rearrange(void)
{
  ...
  if (ok1) {
    ...
    if (ok2) {
      /* FIXME This edit says "j must not be i's parent."
