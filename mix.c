...
void mixrun(void);
void mix(char *infilename, char *intreename, char *mixfilename, char *weightsfilename,
         char *ancfilename, char *outfilename, char *outfileopt, char *outtreename,
         char *outtreeopt, int BestTree, char *Method, int TreeMax, int InputOrder,
         int RandNum, int NumJumble, int OutRoot, int OutNum, int ThreshMix, double ThreshVal,
         int UseAncStates, int SitesWeighted, int AnalyzeMult, int MultDataset, int NumMult,
         int PrintData, int PrintInd, int PrintTree, int PrintSteps, int PrintSeq, int WriteTree);
/* function prototypes */
#endif
...
node2 *root;
long outgrno, msets, ith, njumble, jumb = 0, nonodes = 0;
/*  outgrno indicates outgroup */
long maxtrees;
long inseed, inseed0;
boolean jumble, usertree, weights, thresh, ancvar, questions, allsokal, allwagner, mixture, trout, noroot, outgropt, didreroot, progress, treeprint, stepbox, ancseq, mulsets, firstset, justwts;
...
void mix_findtree(boolean *found, long *pos, long nextree, long *place, long **bestrees)
{
...
void mix_addelement(node2 **p, long *nextnode, long *lparens, boolean *names)
{
...
void describe(void)
{
  /* prints ancestors, steps and table of numbers of steps in each character */
  ...
  if (!usertree)
  {
    ...
    if (jumb == njumble) {
      ...
      for (i = 0; i <= (nextree - 2); i++) {
        ...
        for (j = 3; j <= spp; j++)
          add3(treenode[bestrees[i][j - 1] - 1], treenode[j - 1], treenode[spp + j - 2], &root, treenode);
        if (noroot)
          reroot(treenode[outgrno - 1]);
    ...
    if (numtrees > 2 && chars > 1 ) {
      if (progress)
      {
        sprintf(progbuf, "   sampling for SH test\n");
        print_progress(progbuf);
      }
