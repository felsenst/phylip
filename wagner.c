...
void postorder(node2 *p, long fullset, boolean full, bitptr wagner, bitptr zeroanc)
{
...
void cpostorder(node2 *p, boolean full, bitptr zeroanc, steptr numszero, steptr numsone)
{
...
void filltrav(node2 *r, long fullset, boolean full, bitptr wagner, bitptr zeroanc)
{
...
void hyprint(struct htrav_vars2 *htrav, boolean unknown, boolean noroot, boolean didreroot, bitptr wagner, Char *guess)
{
             ...
             gbit *garbage)
{
  ...
  if (vars.bottom) {
    ...
    memcpy(vars.zerobelow->bits_, treenode[vars.r->back->index - 1]->empstte0, words * sizeof(long));
    memcpy(vars.onebelow->bits_, treenode[vars.r->back->index - 1]->empstte1, words * sizeof(long));
  }
  for (i = 0; i < words; i++) {
    dh = dohyp[i];
    ...
    if (!vars.r->tip) {
      ...
      s0 = (wa & ((a0 & l0) | (a0 & r0) | (l0 & r0))) | (dh & fullset & (~wa) & s0);
      s1 = (wa & ((a1 & l1) | (a1 & r1) | (l1 & r1))) | (dh & fullset & (~wa) & s1);
               ...
               pointptr2 treenode, Char *guess, gbit *garbage)
{
  ...
  for (i = 0; i < chars; i++) {
    ...
    unknown = (unknown || ((((1L << k) & wagner[j - 1]) == 0) && guess[i] == '?'));
  }
