...
extern void winplotpreview(void);
...
long   pagecount;
double oldxmargin,oldymargin, oldhpmargin,oldvpmargin,userxsize,userysize;
...
node* draw_node_new(node_type type, long index)
{
...
void initplotter (long ntips, char *fontname)
{
...
#endif
  (void)fontname;                       // RSGnote: Parameter never used.
  ...
  if (dotmatrix ) {
    for (i = 0; i <= 50; i++) {   /* for fast circle calculations */
      for (j = 0; j <= 50; j++)
      {
        rootmatrix[i][j] = (long)floor(sqrt((double)(i * i + j * j)) + 0.5);
      }
    }
  }
  ...
  switch (plotter) {
    ...
    case xpreview:
#ifdef PHYLIP_CAN_DRAW_WITH_X
      XGetGeometry(display, mainwin, &DefaultRootWindow(display), &x, &inputSequences, &width, &height, &dummy1, &dummy2);
    ...
    case lw:     /* write conforming postscript */
      //printf("calling postscript_header\n");
      //fflush(stdout);
      postscript_header();
      break;

    case svg:      /* write Scaled Vector Graphics */
      svg_height = 792;
      fprintf(plotfile, "<?xml version=\"1.0\" standalone=\"no\"?>\n");
      fprintf(plotfile, "<!DOCTYPE svg PUBLIC \"-//W3C//DTD SVG 1.1//EN\" \n");
      fprintf(plotfile, "\"http://www.w3.org/Graphics/SVG/1.1/DTD/svg11.dtd\">\n\n");

      fprintf(plotfile, "<svg width=\"612\" height=\"792\" version=\"1.1\"\n");
      fprintf(plotfile, "xmlns=\"http://www.w3.org/2000/svg\" xmlns:xlink=\"http://www.w3.org/1999/xlink\">\n\n");
      break;
    ...
    case fig:
      fprintf(plotfile, "#FIG 3.2\n");
      fprintf(plotfile, "Portrait\n");
      fprintf(plotfile, "Center\n");
      fprintf(plotfile, "Inches\n");
      fprintf(plotfile, "Letter\n");
      fprintf(plotfile, "100.00\n");
      fprintf(plotfile, "Single\n");
      fprintf(plotfile, "-2\n");
      fprintf(plotfile, "1200 2\n");
      break;
...
void loadfont(short *font, char *application)
{
  ...
  while (!(eoff(fontfile) || ch == ' ')) {
    charstart = i + 1;
    if(fscanf(fontfile, "%c%c%ld%hd%hd", &ch, &ch, &dummy, &font[charstart + 1], &font[charstart + 2]) < 5)
    {
                 ...
                 long *bottomcolor, long *rx, long *ry, long numtochange)
{
  ...
  if (numtochange == 0) {
    ...
    do {
      ...
      phyFillScreenColor();
      if(scanf("%ld%*[^\n]", &numtochange)) {} // Read number and scan to EOL.
      (void)getchar();
      countup(&loopcount, 10);
  ...
  switch (numtochange) {
    ...
    case 4:
      ...
      if (plotter == pov) {
        ...
        phyFillScreenColor();
        if(scanf("%ld%*[^\n]", rx)) {}  // Read number and scan to EOL.
        (void)getchar();
        printf("Enter the Y resolution:\n");
        phyFillScreenColor();
        if(scanf("%ld%*[^\n]", ry)) {}   // Read number and scan to EOL.
        (void)getchar();
      }
      break;
  }
...
void plotrparms(long ntips)
{
  ...
  switch (plotter) {
    ...
    case fig:
      penchange = yes;
      xunitspercm = 472.4409448818;
      yunitspercm = 472.4409448818;
      xsize = pagex;
      ysize = pagey;
      break;
...
void getplotter(void)
{
  ...
  printf("        S         SVG Scalable Vector Graphics file format\n");
...
#endif
  printf("        F         FIG 3.2 drawing program format          \n");
...
#ifdef DOS
  while (strchr("LSJKHIDBECOTUAZPXRMFWV", ch) == NULL);
#endif
#ifndef DOS
  while (strchr("LSJKHDBECOTAZUPXRMFWV", ch) == NULL);
#endif
  switch (ch) {
    ...
    case 'W':
      ...
      printf("X resolution?\n");
      phyFillScreenColor();
      if(scanf("%lf%*[^\n]", &userxsize)) {} // Read number and scan to EOL.
      (void)getchar();
      printf("Y resolution?\n");
      phyFillScreenColor();
      if(scanf("%lf%*[^\n]", &userysize)) {} // Read number and scan to EOL.
      (void)getchar();
      xunitspercm = 1.0;
      yunitspercm = 1.0;
  ...
  if ((strcmp(fontname, "Helvetica") == 0) ||
    ...
    if (didloadmetric) {
      ...
      if (readafmfile(afmfile, unknown_metric)) {
        ...
        return;}
#ifdef NeXT
      sprintf(afmfile, "%s/Library/Fonts/%s.font/%s.afm", getenv("HOME"), fontname, fontname);
      if (readafmfile(afmfile, unknown_metric)) {
        for (i=31;i<256;++i)
          fontmetric[i-31] = unknown_metric[i-31];
        return;}
      ...
      for (;;) {
        if (javarun)
        {
          strcpy(afmfile, "none"); //java takes care of the metrics
        }
        else
        {
...
double lengthtext(char *pstring, long nchars, char *fontname, fonttype font)
{  /* lengthext */
  long i, j, code;
  static double  sumlength;
  long sumbigunits;
  short afmetric[256];
  //printf("in lengthtext\n");
  //fflush(stdout);
...
void drawit(char *fontname, double *xoffset, double *yoffset, long numlines, node *root)
{
  long i, j, line, xpag, ypag;
  long test_long ;  /* To get a division out of a loop */

  (void)xoffset;                        // RSGnote: currently unused.
  (void)yoffset;                        // RSGnote: currently unused.
...
#define min4(a, b, c, d) (min(min(a, b), min(c, d)))
  ...
  /* print native font.  idraw, PS, pict. */
  else if (plotter == lw) {
    ...
    if (previewing) {
      ...
      MoveTo((int)floor((double)x + 0.5),
             winheight - (long)floor((double)inputSequences + 0.5)+MAC_OFFSET);
      putstring(pstring);
      TextFont(macfontid("courier"));
      TextSize(10);
      TextFace(0);
      ...
      /* txface: */
      fprintf(plotfile, "%c%c", 4, (int)((pictbold ? 1: 0) | (pictitalic ? 2 : 0)| (pictoutline ? 8 : 0)|(pictshadow ? 16 : 0)));
      /* txfloc: */
    ...
    y += text.height * text.cosslope/8.0;
    x -= text.height * text.sinslope/8.0;

    if (javarun)
    {
      // convert font name into svg compatible description
      char svgfont[12];
      char svgstyle[12];
      char svgweight[12];

      // available fonts in current (12/2012) Java interface
      if (!strcmp(fontname, "Times-Roman"))
      {
        sprintf(svgfont, "serif");
        sprintf(svgstyle, "normal");
        sprintf(svgweight, "normal");
      }
      else if (!strcmp(fontname, "Times-Bold"))
      {
        sprintf(svgfont, "serif");
        sprintf(svgstyle, "normal");
        sprintf(svgweight, "bold");
      }
      else if (!strcmp(fontname, "Times-BoldItalic"))
      {
        sprintf(svgfont, "serif");
        sprintf(svgstyle, "italic");
        sprintf(svgweight, "bold");
      }
      else if (!strcmp(fontname, "Times-Italic"))
      {
        sprintf(svgfont, "serif");
        sprintf(svgstyle, "italic");
        sprintf(svgweight, "normal");
      ...
      {
        sprintf(svgfont, "sans-serif");
        sprintf(svgstyle, "normal");
        sprintf(svgweight, "normal");
      }
      else if (!strcmp(fontname, "Helvetica-Bold"))
      {
        sprintf(svgfont, "sans-serif");
        sprintf(svgstyle, "normal");
        sprintf(svgweight, "bold");
      }
      else if (!strcmp(fontname, "Helvetica-Oblique"))
      {
        sprintf(svgfont, "sans-serif");
        sprintf(svgstyle, "italic");
        sprintf(svgweight, "normal");
      }
      else if (!strcmp(fontname, "Helvetica-BoldOblique"))
      {
        sprintf(svgfont, "sans-serif");
        sprintf(svgstyle, "italic");
        sprintf(svgweight, "bold");
      }
      else if (!strcmp(fontname, "Helvetica-Narrow"))
      {
        sprintf(svgfont, "sans-serif");
        sprintf(svgstyle, "normal");
        sprintf(svgweight, "normal");
      }
      else if (!strcmp(fontname, "Helvetica-Narrow-Oblique"))
      {
        sprintf(svgfont, "sans-serif");
        sprintf(svgstyle, "italic");
        sprintf(svgweight, "normal");
      }
      else if (!strcmp(fontname, "Helvetica-Narrow-Bold"))
      {
        sprintf(svgfont, "sans-serif");
        sprintf(svgstyle, "normal");
        sprintf(svgweight, "bold");
      }
      else if (!strcmp(fontname, "Helvetica-Narrow-BoldOblique"))
      {
        sprintf(svgfont, "sans-serif");
        sprintf(svgstyle, "italic");
        sprintf(svgweight, "bold");
      }
      else if (!strcmp(fontname, "Courier"))
      {
        sprintf(svgfont, "monospace");
        sprintf(svgstyle, "normal");
        sprintf(svgweight, "normal");
      }
      else if (!strcmp(fontname, "Courier-Bold"))
      {
        sprintf(svgfont, "monospace");
        sprintf(svgstyle, "normal");
        sprintf(svgweight, "bold");
      }
      else if (!strcmp(fontname, "Courier-Oblique"))
      {
        sprintf(svgfont, "monospace");
        sprintf(svgstyle, "italic");
        sprintf(svgweight, "normal");
      }
      else if (!strcmp(fontname, "Courier-BoldOblique"))
      {
        sprintf(svgfont, "monospace");
        sprintf(svgstyle, "italic");
        sprintf(svgweight, "bold");
      }
      else if (!strcmp(fontname, "AvantGarde-Book"))
      {
        sprintf(svgfont, "sans-serif");
        sprintf(svgstyle, "normal");
        sprintf(svgweight, "normal");
      }
      else if (!strcmp(fontname, "AvantGarde-BookOblique"))
      {
        sprintf(svgfont, "sans-serif");
        sprintf(svgstyle, "italic");
        sprintf(svgweight, "normal");
      }
      else if (!strcmp(fontname, "AvantGarde-Demi"))
      {
        sprintf(svgfont, "sans-serif");
        sprintf(svgstyle, "normal");
        sprintf(svgweight, "bold");
      }
      else if (!strcmp(fontname, "AvantGarde-DemiOblique"))
      {
        sprintf(svgfont, "sans-serif");
        sprintf(svgstyle, "italic");
        sprintf(svgweight, "bold");
      }
      else if (!strcmp(fontname, "Bookman-Light"))
      {
        sprintf(svgfont, "serif");
        sprintf(svgstyle, "normal");
        sprintf(svgweight, "normal");
      }
      else if (!strcmp(fontname, "Bookman-LightItalic"))
      {
        sprintf(svgfont, "serif");
        sprintf(svgstyle, "italic");
        sprintf(svgweight, "normal");
      }
      else if (!strcmp(fontname, "Bookman-Demi"))
      {
        sprintf(svgfont, "serif");
        sprintf(svgstyle, "normal");
        sprintf(svgweight, "bold");
      }
      else if (!strcmp(fontname, "Bookman-DemiItalic"))
      {
        sprintf(svgfont, "serif");
        sprintf(svgstyle, "italic");
        sprintf(svgweight, "bold");
      }
      else if (!strcmp(fontname, "NewCenturySchlbk-Roman"))
      {
        sprintf(svgfont, "serif");
        sprintf(svgstyle, "normal");
        sprintf(svgweight, "normal");
      }
      else if (!strcmp(fontname, "NewCenturySchlbk-Italic"))
      {
        sprintf(svgfont, "serif");
        sprintf(svgstyle, "italic");
        sprintf(svgweight, "normal");
      }
      else if (!strcmp(fontname, "NewCenturySchlbk-Bold"))
      {
        sprintf(svgfont, "serif");
        sprintf(svgstyle, "normal");
        sprintf(svgweight, "bold");
      }
      else if (!strcmp(fontname, "NewCenturySchlbk-BoldItalic"))
      {
        sprintf(svgfont, "serif");
        sprintf(svgstyle, "italic");
        sprintf(svgweight, "bold");
      }
      else if (!strcmp(fontname, "Palatino-Roman"))
      {
        sprintf(svgfont, "serif");
        sprintf(svgstyle, "normal");
        sprintf(svgweight, "normal");
      }
      else if (!strcmp(fontname, "Palatino-Italic"))
      {
        sprintf(svgfont, "serif");
        sprintf(svgstyle, "italic");
        sprintf(svgweight, "normal");
      }
      else if (!strcmp(fontname, "Palatino-Bold"))
      {
        sprintf(svgfont, "serif");
        sprintf(svgstyle, "normal");
        sprintf(svgweight, "bold");
      }
      else if (!strcmp(fontname, "Palatino-BoldItalic"))
      {
        sprintf(svgfont, "serif");
        sprintf(svgstyle, "italic");
        sprintf(svgweight, "bold");
      }
      else if (!strcmp(fontname, "ZapfChancery-MediumItalic"))
      {
        sprintf(svgfont, "serif");
        sprintf(svgstyle, "italic");
        sprintf(svgweight, "normal");
      ...
      fprintf(plotfile, "<text transform=\"translate(%.2f,%.2f) rotate(%.2f)\" font-size=\"%.1f\" font-family=\"%s\" font-style=\"%s\" font-weight=\"%s\">%s</text>\n",
              x, svg_height - y, slope, pointsize, svgfont, svgstyle, svgweight, pstring);
    }
    else
    {
      fprintf(plotfile, "<text transform=\"translate(%.2f,%.2f) rotate(%.2f)\" font-size=\"%.1f\" font-family=\"%s\">%s</text>\n",
              x, svg_height - y, slope, pointsize, fontname, pstring);
    }
  }
...
/* fn--fontname| xo, yo--x and y offsets */
{
  ...
  (void)ntips;                          // RSGnote: currently unused.
...
*/
  ...
  for (i=0; i<xpag; ++i) {
    ...
    if (i != 0) {
      plot(penup, (xscale * (*xo)) +xscale*i*(paperx - hpmargin)*xpagecorrection+xscale*hpmargin, ((*yo)*yscale));
  ...
  for (j=0;j<ypag;++j) {
    plot(penup, (xscale * (*xo)), ((*yo)*yscale)+yscale*j*(papery-vpmargin)*ypagecorrection);
    plot(pendown, (xscale * (*xo))+xscale*pagex*xpagecorrection, ((*yo)*yscale)+yscale*j*(papery-hpmargin)*ypagecorrection);
    if (j != 0) {
      plot(penup, (xscale * (*xo)), ((*yo)*yscale)+yscale*j*(papery-vpmargin)*ypagecorrection+yscale*vpmargin);
      plot(pendown, (xscale * (*xo))+xscale*pagex*xpagecorrection, ((*yo)*yscale)+yscale*j*(papery-hpmargin)*ypagecorrection+yscale*vpmargin);
    }
  }
}  /* makebox */
...
void do_dialog(void)
{
  if (shell != NULL)
    return;
  shell = XtCreatePopupShell("About", transientShellWidgetClass, toplevel, NULL, 0);
  dialog = XtCreateManagedWidget("dialog", dialogWidgetClass, shell, NULL, 0);
  XawDialogAddButton(dialog, "Dismiss", (XtCallbackProc)dismiss_dialog, NULL);
  XtRealizeWidget(shell);
  wm_delete_window2 = XInternAtom(XtDisplay(shell), "WM_DELETE_WINDOW", 0);
  XSetWMProtocols(XtDisplay(shell), XtWindow(shell), &wm_delete_window2, 1);
  XtMapWidget(shell);
}
...
void init_x(void)
{
  ...
  toplevel=XtAppInitialize(&appcontext, "phylip", NULL, 0, &nargc, nargv, res, NULL, 0);
  ...
  XGetGeometry(XtDisplay(toplevel), XDefaultRootWindow(XtDisplay(toplevel)),
               &dummy3, &x, &inputSequences, &width, &height, &dummy1, &dummy2);
  ...
  drawing_area = XtCreateManagedWidget("drawing_area", coreWidgetClass, paned, wargs, 2);

  /* create a menubuton add it to the menubar*/
  menuButton = XtCreateManagedWidget ("File", menuButtonWidgetClass, menubar, NULL, 0);
  ...
  entry=XtCreateManagedWidget("Change Parameters", smeBSBObjectClass, menu, NULL, 0);
  ...
  menuButton = XtCreateManagedWidget("Help", menuButtonWidgetClass, menubar, NULL, 0);
  ...
  gc1=XCreateGC (XtDisplay (toplevel), XtWindow (drawing_area), GCForeground, &values);

  mainwin=XtWindow(drawing_area);

  XtAddEventHandler(drawing_area, ExposureMask, FALSE, (XtEventHandler)redraw, NULL);

  XtAddEventHandler(toplevel, StructureNotifyMask, FALSE, (XtEventHandler)redraw, NULL);
