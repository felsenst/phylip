...
public class ProMLKUserInterface {
   public interface ProMLK extends Library {
        public void promlk(
        		String infile,
        		String intree,
        		String wgtsfile,
	...
	private JFrame frmProMLKControls;
	...
	public static void main(final String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					ProMLKUserInterface window = new ProMLKUserInterface(args);
					window.frmProMLKControls.setVisible(true);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	...
	protected void ChooseFile(JTextField file) {
		...
		int option = fileChooser.showOpenDialog(frmProMLKControls.getRootPane());
	 ...
	 */
	public ProMLKUserInterface(String[] args) {
		phylipCall = false;
		if (args.length > 0)
		{
	...
	private void initialize() {
		...
		frmProMLKControls = new JFrame();
		frmProMLKControls.setBackground(new Color(204, 255, 255));
		frmProMLKControls.setTitle("Promlk");
		frmProMLKControls.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		frmProMLKControls.setBounds(100, 100, 1200, 700);
		frmProMLKControls.setPreferredSize(new Dimension(frmProMLKControls.getBounds().width, frmProMLKControls.getBounds().height));
		
		scrollPane = new JScrollPane();
		scrollPane.setPreferredSize(frmProMLKControls.getPreferredSize());
		frmProMLKControls.getContentPane().add(scrollPane);
		
		panel = new JPanel();
		panel.setPreferredSize(frmProMLKControls.getPreferredSize());
		...
		cmbxHMMcount.setEnabled(false);
		panel.add(cmbxHMMcount, "cell 2 15,growx");
		...
		cmbxHMMCat.setEnabled(false);
		panel.add(cmbxHMMCat, "cell 3 15,growx");
		...
		lblHMMProb = new JLabel("Probablilty:");
		lblHMMProb.setHorizontalAlignment(SwingConstants.RIGHT);
		lblHMMProb.setFont(new Font("Arial", Font.BOLD, 13));
		lblHMMProb.setEnabled(false);
		panel.add(lblHMMProb, "cell 3 16");
		...
		panel.add(lblAnalyzeMultData, "cell 4 10,alignx right");
		...
		rdbtnAnalyzeMultDataYes.setSelected(false);
		panel.add(rdbtnAnalyzeMultDataYes, "cell 5 10");
		...
		rdbtnDataSets.setEnabled(false);
		panel.add(rdbtnDataSets, "cell 5 11");
		...
		rdbtnWeights.setEnabled(false);
		panel.add(rdbtnWeights, "cell 5 11");
		...
		lblHowManyData.setEnabled(false);
		panel.add(lblHowManyData, "cell 5 12");
		...
		panel.add(txtNumSeqs, "cell 5 12");
		...
		panel.add(lblInputSeq, "cell 4 13,alignx right");
		...
		rdbtnInputSeqYes.setSelected(true);
		panel.add(rdbtnInputSeqYes, "cell 5 13");
		...
		rdbtnInputSeqNo.setSelected(false);
		panel.add(rdbtnInputSeqNo, "cell 5 13");
		...
		panel.add(lblPrintData, "cell 4 14,alignx right");
		...
		rdbtnPrintDataYes.setSelected(false);
		panel.add(rdbtnPrintDataYes, "cell 5 14");
		...
		rdbtnPrintDataNo.setSelected(true);
		panel.add(rdbtnPrintDataNo, "cell 5 14");
		...
		panel.add(lblPrintTree, "cell 4 15,alignx right");
		...
		rdbtnPrintTreeYes.setSelected(true);
		panel.add(rdbtnPrintTreeYes, "cell 5 15");
		...
		rdbtnPrintTreeNo.setSelected(false);
		panel.add(rdbtnPrintTreeNo, "cell 5 15");
		...
		panel.add(lblWriteTree, "cell 4 16,alignx right");
		...
		rdbtnWriteTreeYes.setSelected(true);
		panel.add(rdbtnWriteTreeYes, "cell 5 16");
		...
		rdbtnWriteTreeNo.setSelected(false);
		panel.add(rdbtnWriteTreeNo, "cell 5 16");
		...
		panel.add(lblRecHypo, "cell 4 17,alignx right");
		...
		rdbtnRecHypoYes.setSelected(false);
		panel.add(rdbtnRecHypoYes, "cell 5 17");
		...
		rdbtnRecHypoNo.setSelected(true);
		panel.add(rdbtnRecHypoNo, "cell 5 17");
		...
		panel.add(lblPrintInd, "cell 4 18,alignx right");
		...
		rdbtnPrintIndYes.setSelected(true);
		panel.add(rdbtnPrintIndYes, "cell 5 18");
		...
		rdbtnPrintIndNo.setSelected(false);
		panel.add(rdbtnPrintIndNo, "cell 5 18");
		...
		btnExecute.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (checkInputVals())
				{
			  	    ...
			  	    Thread proMLKThread = new Thread() {
						public void run() {
							runProMLKThreads();
						}
			  	    };
		...
		panel.add(btnExecute, "cell 5 19");
	...
	protected void runProMLKThreads() {
    	try
    	{
    		// see if library exists
    		Native.loadLibrary("promlk", ProMLK.class);
		}
    	catch(UnsatisfiedLinkError e)
    	{
     		new TestFileNames().LibraryMissing("ProMLK");
    		return;
    	}
		try 
		{
	  	    Thread proMLKRunThread = new Thread() {
		  	      public void run() {
		  	    	  
		  			// at this point we hook into the C code
		  			ProMLK promlk = (ProMLK) Native.loadLibrary("promlk", ProMLK.class);
		  	        promlk.promlk(		
		  	        		inputvals.infile,
		  	        		inputvals.intree,
		  	        		inputvals.wgtsfile,
