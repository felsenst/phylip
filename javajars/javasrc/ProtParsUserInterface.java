...
public class ProtParsUserInterface {
   public interface ProtPars extends Library {
        public void protpars(
        		String infile,
        		String intree,
        		String outfile,
        		String outfileopt,
	...
	private ProtParsData inputvals;
	...
	private JFrame frmProtParsControls;
	...
	public static void main(final String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					ProtParsUserInterface window = new ProtParsUserInterface(args);
					window.frmProtParsControls.setVisible(true);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	...
	protected void ChooseFile(JTextField file) {
		...
		int option = fileChooser.showOpenDialog(frmProtParsControls.getRootPane());
	 ...
	 */
	public ProtParsUserInterface(String[] args) {
		phylipCall = false;
		if (args.length > 0)
		{
	...
	private void initialize() {
		...
		frmProtParsControls = new JFrame();
		frmProtParsControls.setBackground(new Color(204, 255, 255));
		frmProtParsControls.setTitle("Protpars");
		frmProtParsControls.setBounds(100, 100, 630, 860);
		frmProtParsControls.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		frmProtParsControls.setPreferredSize(new Dimension(frmProtParsControls.getBounds().width, frmProtParsControls.getBounds().height));
		
		scrollPane = new JScrollPane();
		scrollPane.setPreferredSize(frmProtParsControls.getPreferredSize());
		frmProtParsControls.getContentPane().add(scrollPane);
		
		panel = new JPanel();
		panel.setPreferredSize(frmProtParsControls.getPreferredSize());
		...
		rdbtnRandOrderNo.setSelected(true);
		panel.add(rdbtnRandOrderNo, "cell 2 7");
		...
		rdbtnOutNo.setSelected(true);
		panel.add(rdbtnOutNo, "cell 2 10");
		...
		rdbtnThresholdYes.setSelected(false);
		panel.add(rdbtnThresholdYes, "cell 2 12");
		...
		rdbtnThresholdNo.setSelected(true);
		panel.add(rdbtnThresholdNo, "cell 2 12");
		...
		panel.add(lblGenCode, "flowx,cell 0 14 2 1,alignx right");
		...
		panel.add(cmbxGenCode, "cell 2 14, growx");
		...
		panel.add(lblPrintSeq, "flowx,cell 0 23 2 1,alignx right");
		...
		rdbtnPrintSeqYes.setSelected(false);
		panel.add(rdbtnPrintSeqYes, "cell 2 23");
		...
		rdbtnPrintSeqNo.setSelected(true);
		panel.add(rdbtnPrintSeqNo, "cell 2 23");
		...
		btnExecute.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String title = "Protpars Progress";
				...
				if (checkInputVals())
				{
			  	    ...
			  	    Thread protParsThread = new Thread() {
						public void run() {
							runProtParsThreads();
						}
			  	    };
			  	    protParsThread.start();
				}
				btnExecute.setEnabled(true);
				}
		});
		btnExecute.setFont(new Font("Arial", Font.BOLD, 13));
		panel.add(btnExecute, "cell 2 26");
	...
	protected void runProtParsThreads() {
    	try
    	{
    		// see if library exists
    		Native.loadLibrary("protpars", ProtPars.class);
		}
    	catch(UnsatisfiedLinkError e)
    	{
     		new TestFileNames().LibraryMissing("ProtPars");
    		return;
    	}
		try 
		{
	  	    Thread protParsRunThread = new Thread() {
		  	      public void run() {
		  	    	  
		  			// at this point we hook into the C code
		  			ProtPars protpars = (ProtPars) Native.loadLibrary("protpars", ProtPars.class);
		  	        protpars.protpars(
		  	       		inputvals.infile,
		  	       		inputvals.intree,
		  	       		inputvals.outfile,
	  	    ...
	  	    if (inputvals.PrintInd)
	  	    {
					...
					Thread.sleep(1000);
					updateProgress();
				} while (protParsRunThread.isAlive());
	  	    }
		} 
