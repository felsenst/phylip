/* version 3.7. (c) Copyright 2009 by the University of Washington.
...
void   codml_empiricalfreqs(double *, double *, double *,
         double *, steptr, pointarray);
...
boolean codml_good_node(tree *t, node *n);
...
void   codml_unroot(tree *);
void   initcodonmlnode(node **, node **, node *, long, long, long *, long *,
                      initops, pointarray, pointarray, Char *, Char *, FILE *);
void   maketree (void);
void   clean_up(void);
...
long nonodes2, numsensecodons, dataSites, workingSites, weightsum, rcategs,
  datasets, ith, njumble, jumb;
long inseed, inseed0, parens;
boolean f84, hky, k2p, jc,global, jumble, weights, trout, usertree, reusertree,
  rctgry, auto_, freqsfrom, hypstate, progress, mulsets, justwts, firstset,
  improve, smoothit, polishing, lngths, gama, invar, inputDataIsNucleotideData;
...
void debugtree(tree*);   /* debug -- its prototype */
void debugtree(tree* t)
{
   ...
   for(i=0;i<nonodes2;i++) {
      ...
      printf("%3s nodep[%d] == %p ->index = %ld ",
          p->tip > 0 ? "TIP" : "",i,p,p->index);
...
void codml_empiricalfreqs(double *freqa, double *freqc, double *freqg,
...
void getoptions()
{
  ...
  for (;;){
    ...
    if (((!usertree) && !rctgry && (strchr("UKTZDFQNWSGJOMI012345", ch) != NULL))
      ...
      // EWFIX.BUG -- this is overwritten too soon to be seen
...
void aa_codon_makevalues(long rcategs, pointarray treenode, long endsite,
      ...
      for (l = 0; l < rcategs; l++) {
        ...
        switch (y[i][j - 1]) {
          ...
          case 'Z':
              ...
              break;
          default:
            printf("ERROR:  Unexpected amino acid symbol \'%c\' created internally; program error.",
                     y[i][j - 1]);
              exxit(-1);
        }
        ...
        treenode[i]->initialized = 1; /* EWFIX */
      }
    }
  }
}  /* aa_codon_makevalues */
...
void nuc_codon_makevalues(long rcategs, pointarray treenode, long endsite,
                        long spp, sequence y, steptr alias)
{
  /* set up fractional likelihoods at tips         */
      ...
      for (l = 0; l < rcategs; l++) {
        ...
        for (p = 0; p < NUM_NUCS_IN_CODON; p++)
          ...
          case '-':
            ...
            break;
          default:
            printf("ERROR:  Unexpected nucleotide symbol \'%c\' created internally; program error.",
                     y[i][3*k + p]);
              exxit(-1);
          }
        for (m = 0; m < NUM_NUCLEOTIDE_STATES; m++)
...
/* read or compute the base frequencies */
  ...
  if (hky || f84) {
    if (!freqsfrom)
      ...
      codml_empiricalfreqs(&nucFreq[0], &nucFreq[1], &nucFreq[2], &nucFreq[3],
                           weight, curtree->nodep);
  }
  else {
    nucFreq[0] = 0.25;
    nucFreq[1] = 0.25;
...
boolean codonfreqs_sum_to_one(void); /* debug -- its prototype */
boolean codonfreqs_sum_to_one()
{
...
boolean is_f_times_m_symmetric(double **);  /* debug --- its prototype */
...
{ /* QR eigenvector/eigenvalue method for symmetric matrix       */
  ...
  /* EWFIX -- change so we use eig instead of eigmat ?? */
  ...
  for (i = 0; i < n; i++)
    {
      for (j = 0; j < n; j++)
      {
        ...
        /* EWFIX -- original had conversion using codonfreqs
...
void getinput()
{
  ...
  // EWFIX.CHECK -- took this away as other tests are inside inputoptions -- if (!justwts || firstset)
  inputoptions();
  if (!justwts || firstset)
  ...
  // EWFIX.CHECK -- probably OK already -- setuptree2(&curtree);
  /*
  if (!usertree) {
    setuptree2(&bestree);
  ...
  */
  /* EWFIX.CHECK -- these are probably being handled by the "new" phylip c++-like scheme --
...
void codon_tree_calc_nuview(tree * t, node * p, cphenotype cph, double *  underflows)
{
  ...
  for (sib_index=0; sib_index < num_sibs; sib_index++) {
    ...
    for (j = 0; j < rcategs; j++)
      make_pmatrix(pmatrices[sib_index][j], NULL, NULL, 0, lw,
                                        tbl[j], eigmat[j], probmat[j]);
  }
...
void codon_tree_nuview(tree* t, node *p)
{
  ...
  for ( siteIndex = 0 ; siteIndex < endsite ; siteIndex++ )
      ...
      {
          memcpy(((codon_node*)p)->codonx[siteIndex][rcategIndex],
            temp_codonx[siteIndex][rcategIndex],
            sizeof(csitelike));
      }
...

{
  ...
  for (i = 0; i < endsite; i++) {
    ...
    /* EWFIX -- don't want to use this new sumterm above */
...
void codon_tree_makenewv(tree* t, node *p)
{
  ...
  /* EWFIX -- is there a better place to make sure it's always done? */
  inittrav(p);
  assert(q == p->back);
  inittrav(p->back);
...
void codml_restore_traverses(tree *t, node *p, node * q)
{
    /* EWFIX -- should this be ml's version (which doesn't
       exist, but could some day? */
    generic_tree_restore_traverses(t,p,q);

    /* EWFIX -- moved updating of {p,q}->back-> to the generic
     * version */
}
...
void make_pmatrix(double **mymatrix, double **dmat, double **ddmat,
                        long derivative, double lz, double rat,
                        double *eigm, double **probm)
{
  ...
  for (k = 0; k < numsensecodons; k++) {
    ...
    /* EWFIX.CHECK -- can this be 0.0? any upper limit?*/
    ...
    if ((ally[n] == 0) || (location[ally[n]-1] == 0))
      ...
      for (i = 0; i <= 60; i++)
        if (xx[i] > xx[first])
          first = i;
      if (xx[first] > 0.95)
        fprintf(outfile, "%s ", codonStrings[first]); // EWFIX -- check
      else 
      {
        ...
        fprintf(outfile, "%s ", lowCodon); // EWFIX -- check
      }
      if (rctgry && rcategs > 1)
        mx = mp[n][mx - 1];
      else
...
void summarize()
{
  ...
  if (rctgry && rcategs > 1) {
    ...
    mx0 = mx;
    fprintf(outfile,
 "Combination of omega values that contributes the most to the likelihood:\n\n");
...
void codml_unroot(tree* t) 
{
  node *r,*q;
  double newl;

  long numsibs = count_sibs(t->root);
  ...
  if ( numsibs > 2) {
    ...
    newl = t->root->next->oldlen + t->root->next->next->oldlen;
    t->root->next->back->oldlen = newl;
    t->root->next->next->back->oldlen = newl;

    newl = t->root->next->v + t->root->next->next->v;
    t->root->next->back->v = newl;
    t->root->next->next->back->v = newl;

    t->root->next->back->back = t->root->next->next->back;
    t->root->next->next->back->back = t->root->next->back;
    t->release_fork(t,t->root);
  }
} /* codml_unroot */


void initcodonmlnode(node **p, node **grbg, node *q, long len, long nodei,
		     long *ntips, long *parens, initops whichinit,
		     pointarray treenode, pointarray nodep, Char *str,
		     Char *ch, FILE *intree)
{
...
void maketree()
{
  ...
  if (usertree) {
    ...
    for( which = 1 ; which <= numtrees; which++) {
      ...
      goteof           = false;
      preparetree(curtree);
      treeread(intree, &curtree->root, dummy_treenode, &goteof, &dummy_first,
                      curtree->nodep, &nextnode, &haslengths, &grbg,
                      initcodonmlnode,false,nonodes2);
      fixtree(curtree);
      codml_unroot(curtree);
      ...
      ml_treevaluate(curtree,improve,reusertree,global,progress,
            priortree,bestree,ml_inittravtree);
      // EWFIX -- proml has reusertree stuff in here
      if (treeprint) {
        codml_printree();
        summarize();
      }
    ...
    while (nextsp <= spp) {
      ...
      curtree->addtraverse(curtree,curtree->nodep[enterorder[nextsp - 1] - 1],
                  curtree->root, true,&qwhere,&bestyet,bestree,priortree,smoothit,NULL);
      ...
      assert(curtree->tree_good_f(curtree)); /* EWFIX */
