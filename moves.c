...
void stwrite(const char *s, long length, long *pos, long leftedge, long screenwidth)
{
  if ((*pos) >= leftedge && (*pos) - leftedge + 1 < screenwidth)
...
void treeoptions(boolean waswritten, Char *ch, FILE **outtree, Char *outtreename, Char *progname)
{ /* interactively get options for writing a tree */
  ...
  {
    openfile(outtree, outtreename, "output tree file", (*ch == 'A' && waswritten) ? "a" : "w", progname, outtreename);
  }
}  /* treeoptions */
