...
void drawfatline(long ixabs, long iyabs, long ixnow, long iynow, long penwide)
{
...
{
  long xhigh, yhigh, xlow, ylow, ixnow, iynow, ixabs, iyabs, cdx, cdy, temp, i;
  long pictint;
    ...
    switch (plotter)
    {
      case xpreview:
        if (pen == pendown) {
          ...
          XDrawLine(display,mainwin,gc1,(long)oldx,(long)(height-oldy), (long)xabs,(long)(height-yabs));
#endif
        }
        oldx = xabs;
        oldy = yabs;
      ...
      case svg:
        if (pen == pendown)
        {
          fprintf(plotfile, "<path stroke=\"black\" stroke-linecap=\"round\" stroke-linejoin=\"round\" stroke-width=\"%.1f\" d=\"M %8.2f %8.2f L %8.2f %8.2f\" fill=\"none\" />\n",
                  treeline, oldx, svg_height - oldy, xabs, svg_height - yabs);
        }
        oldx     = xabs,
          oldy     = yabs;
        break;
      ...
      case fig:
        newx = floor(xsize*xabs + 0.5);
        newy = floor(ysize*yunitspercm - ysize*yabs + 0.5);
        if (pen == pendown) {
          fprintf(plotfile, "2 1 0 %5ld 0 -1 50 0 -1 0.000 1 1 -1 0 0 2\n",
                  (long)floor(linewidth + 0.5) + 1);
          fprintf(plotfile, "%ld %ld %ld %ld\n",
                  (long)oldx, (long)oldy, (long)newx, (long)newy);
        }
        oldx = newx;
        oldy = newy;
        break;
            ...
            long segs, boolean head, boolean tail)
{
  ...
  switch (plotter)
  {
    ...
    case svg:
      fprintf(plotfile,"<path stroke=\"black\" stroke-width=\"%.1f\" d=\"M %8.2f,%8.2f ", treeline, x1, svg_height-y1);
      if (sense)
        fprintf(plotfile,"C %8.2f,%8.2f %8.2f,%8.2f %8.2f,%8.2f",
                (x1+(0.55*(x2-x1))), svg_height-y1, x2, svg_height-(y1+(0.45*(y2-y1))),
                x2, svg_height-y2);
      else
        fprintf(plotfile,"C %8.2f %8.2f %8.2f %8.2f %8.2f %8.2f",
                x1, svg_height-(y1+(0.55*(y2-y1))), (x1+(0.45*(x2-x1))), svg_height-y2,
                x2, svg_height-y2);
      fprintf(plotfile,"\" fill=\"none\" />\n");
      break;
    ...
    case fig:
      fromx = (long)floor(xsize*x1 + 0.5);
      fromy = (long)floor(ysize*yunitspercm - ysize*y1 + 0.5);
      tox = (long)floor(xsize*x2 + 0.5);
      toy = (long)floor(ysize*yunitspercm - ysize*y2 + 0.5);
      ...
      if (sense)
        fprintf(plotfile, "%5ld%5ld%5ld%5ld%5ld%5ld%5ld%5ld 9999 9999\n",
                fromx, fromy,
                (long)floor((x1+(0.55*(x2-x1))) + 0.5), fromy,
                tox, (long)floor((y1+(0.45*(y2-y1))) + 0.5),
                tox, toy);
      else
        fprintf(plotfile, "%5ld%5ld%5ld%5ld%5ld%5ld%5ld%5ld 9999 9999\n",
                fromx, fromy,
                fromx, (long)floor((y1+(0.55*(y2-y1))) + 0.5),
                (long)floor((x1+(0.45*(x2-x1))) + 0.5), toy,
                tox, toy);
      fprintf(plotfile, "1 3 0  1 0 0 0 21 0.00 1 0.0 ");
      fprintf(plotfile, "%5ld%5ld%5ld %5ld %5ld%5ld%5ld 349\n",
              fromx, fromy, (long)floor(linewidth / 2 + 0.5),
              (long)floor(linewidth / 2 + 0.5), fromx,
              fromy, fromy);
      fprintf(plotfile, "1 3 0  1 0 0 0 21 0.00 1 0.0 ");
      fprintf(plotfile, "%5ld%5ld%5ld %5ld %5ld%5ld%5ld 349\n",
              tox, toy, (long)floor(linewidth / 2 + 0.5),
...
void swoopspline(double x1, double y1, double x2, double y2, double x3, double y3, boolean sense, long segs)
{
...
void curvespline(double x1, double y1, double x2, double y2, boolean sense, long segs)
{
...
double computeAngle(double oldx, double oldy, double newx, double newy)
{
  ...
  if ((newx-oldx) == 0 )
  {
    ...
    else
    {
      ...
      fprintf(stderr, "ERROR:  Angle can't be computed, 2 points on top of each other in computeAngle()!\n");
      angle = 0;
    }
  }
  else
...
/*******************  Global Variables ********************/
extern void winplotpreviewcore(void);
HANDLE ghInstance;
