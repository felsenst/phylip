/* Version 4.0. (c) Copyright 1988-2013 by the University of Washington.
...
#define sizearray       80000 /* Size of symbarray; must be >= the sum of     */
...
void   factorrun(void);
void   factor(char *infilename, char *outfilename, char *ancfilename, char *factfilename,
              int  outputAncStates, int  outputFactors, int  printInd, char *outfileopt);
/* function prototypes */
#endif
  ...
  for (i = 0; i < (nstates); i++) {
    if (nodes[i] != root) {
      if (nodes[i] == NULL) {
        ...
        if (poynter != root) {
          printf("ERROR:  Character %ld: invalid character state tree description.\n\n", charnumber);
          exxit(-1);}
      }
    }
  }
    ...
    if (numstates[charindex] == 0)
      ...
      if (i > numstates[charindex]) {
        if( multichar[charnum[charindex] - 1] == unkchar) {
          ...
          putc('\n', outfile);
          printf("\nERROR:  In species %ld, multistate character %ld:  ", eu, charnum[charindex]);
          printf("'%c' is not a documented state.\n\n", multichar[charnum[charindex] - 1]);
          exxit(-1);
        }
      } else {
        place = chstart[charindex] + numstates[charindex] + (numstates[charindex] - 1) * (i - 1);
...
void factorrun(void)
{
  ...
  if(fscanf(infile, "%ld%ld", &neus, &nchars) < 2)
  {
    printf("\nERROR reading input file.\n\n");
    exxit(-1);
  }
  ...
  char *outfileopt)
{
  ...
  */
  ...
  if (ancstrrequest) {
    //printf("ancfile: %s \n", ancfilename); // JRMdebug
    outancfile = fopen(ancfilename, "w");
  }
  if (factorrequest) {
    //printf("factfile: %s \n", factfilename); // JRMdebug
    outfactfile = fopen(factfilename, "w");
  }

  factorrun(); // do the actual work
...
int main(int argc, Char *argv[])
{
  ...
  getoptions();
  if(factorrequest)
    openfile(&outfactfile, "factors", "output factors file", "w", argv[0], outfactfilename);
  if(ancstrrequest)
    openfile(&outancfile, "ancestors", "output ancestors file", "w", argv[0], outancfilename);
