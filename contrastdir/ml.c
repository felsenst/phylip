...
extern long endsite, nextree, which; 
...
static boolean ml_tree_try_insert_thorough(tree*, node*, node*, node **, 
    double*, tree*, tree*);
static boolean ml_tree_try_insert_notthorough(tree *, node *, node *, node**,
    double*);
...
{ /* make a new  ml_tree  */
  ml_tree* t = Malloc(sizeof(ml_tree));
  ml_tree_init((tree*)t, nonodes, spp);
  return t;
} /* ml_tree_new */
...
void ml_node_copy(node* srcn, node* destn)
{ /* copy an ml_node */
  ...
  memcpy(dest->underflows,src->underflows,endsite*sizeof(double));
} /* ml_node_copy */
...
{ /* copy a codon_node */
  ...
  if ( oldendsite == 0 ) 
    ((ml_node*)dest)->allocx(((ml_node*)dest), ((ml_node*)src)->endsite,
                             ((ml_node*)src)->categs);

  for (i = 0; i < src->ml_node.endsite; i++)
    for (j = 0; j < src->ml_node.categs; j++)
      memcpy(dest->codonx[i][j], 
          src->codonx[i][j], sizeof(csitelike)); 
...
void dna_node_copy(node* srcn, node* destn)
{
  ...
  for (i = 0; i < ((ml_node*)src)->endsite; i++)
    for (j = 0; j < ((ml_node*)src)->categs; j++)
      memcpy(((dna_node*)dest)->x[i][j], 
          ((dna_node*)src)->x[i][j], sizeof(sitelike)); 
...
{ /* used in Proml, Promlk */
  ...
  for ( i = 0 ; i < rcategs  ; i++ ) 
    for (m = 0; m < 61; m++)  /* EWFIX -- should use NUM_SENSE_CODONS */
    {
      assert(maxx > 0.0 || p->codonx[site][i][m] == 0.0);
      if (maxx > 0.0) p->codonx[site][i][m] /= maxx;
    }
} /* fix_codonx */
...
node* ml_node_new(boolean tip, long index) 
{
  node* n = Malloc(sizeof(ml_node));
  ml_node_init(n, tip, index);
  return n;
}
...
void ml_node_reinit(node * n)
{
    ml_node * mln = (ml_node*)n;
    mln->node.tyme = 0;
    /* EWFIX.REINIT -- does freex need refreshing ? */
    /* EWFIX.REINIT -- leave for dna_node and prot_node ? */
    ...
    for (i = 0; i < spp; i++) {
      for (l = 0; l < categs; l++) {
        for (b = A; (long)b <= (long)T; b = (bases)((long)b + 1))
          ((dna_node*)treenode[i])->x[k][l][(long)b - (long)A] = 0.0;
        switch (y[i][j - 1]) {

        case 'A':
          ((dna_node*)treenode[i])->x[k][l][0] = 1.0;
          break;

        case 'C':
          ((dna_node*)treenode[i])->x[k][l][(long)C - (long)A] = 1.0;
          break;

        case 'G':
          ((dna_node*)treenode[i])->x[k][l][(long)G - (long)A] = 1.0;
          break;

        case 'T':
          ((dna_node*)treenode[i])->x[k][l][(long)T - (long)A] = 1.0;
          break;
        ...
        case 'N':
          for (b = A; (long)b <= (long)T; b = (bases)((long)b + 1))
            ((dna_node*)treenode[i])->x[k][l][(long)b - (long)A] = 1.0;
          break;
...
void prot_freex(long nonodes, pointarray treenode)
{
  ...
  for (i = 0; i < spp; i++) {
    for (j = 0; j < endsite; j++)
      free(((prot_node*)treenode[i])->x[j]);
    free(((prot_node*)treenode[i])->x);
  }
  for (i = spp; i < nonodes; i++) {
    p = treenode[i];
    do {
...
void update(tree *t, node *p)
{
  ...
  if ((!usertree) || (usertree && !lngths) || p->iter) {
    ...
    /* EWFIX.CHECK -- probably can get rid of all of this
    if ( smoothit ) {
    */
      inittrav(p);
      inittrav(p->back);
...
static void ml_tree_smoothall(tree* t, node* p)
{
...
#ifdef DEBUG
  double before, after; /* For debugging */

  before = t->evaluate(t, t->root, 0); /* DEBUG */
#endif
  ...
  /* it may seam like we are doing too many smooths, but sometimes 
...
#ifdef DEBUG
  after = t->evaluate(t, t->root, 0); /* DEBUG */
  assert( before == UNDEFINED || before <= after );
#endif
}



/* split q->v branch length evenly beween fork->next and fork->next->next */
void ml_tree_do_branchl_on_insert(tree*t, node *fork, node* q) 
{
  double newv;

  /* generic_do_branchl_on_insert(t,fork,q); */

  newv = q->v * 0.5;
  
  /*
   * fork should be where node was inserted
   * set to initial v for *both* directions
   */
  fork->v = initialv;
  fork->back->v = initialv;

  /* fork->next for both directions
   */
  fork->next->v = newv ;
  fork->next->back->v = newv ;

  /* fork->next->next for both directions
   */
  fork->next->next->v = newv;
  fork->next->next->back->v = newv;

  /* EWFIX.INIT -- might consider invalidating views here or in generic */
  inittrav(fork);
  inittrav(fork->back);
  inittrav(fork->next);
  inittrav(fork->next->back);
  inittrav(fork->next->next);
  inittrav(fork->next->next->back);
}
...
{ 
  ...
  node* r;
      ...
      /* EWFIX.TREECHECK
      printf("EWFIX.INSERT startof %p %p\n",p,q);
      t->tree_print_f(t);
  ...
  generic_tree_insert_(t, p, q, dooinit, false); /*no multifurcate on ml insert_*/
      ...
      /* EWFIX.TREECHECK
      printf("EWFIX.INSERT middlof %p %p\n",p,q);
      t->tree_print_f(t);
      assert(t->tree_good_f(t));
      */
  ...
  if ( !dooinit )  {
    ...
    for ( i = 0 ; i < smoothings ; i++) {
      ...
      /* EWFIX.TREECHECK
      printf("EWFIX.INSERT end  of %p %p\n",p,q);
      t->tree_print_f(t);
      assert(t->tree_good_f(t));
      */
}  
...
void ml_tree_do_branchl_on_re_move(tree* t, node* p, node*q)
{
  ...
  /*
   * EWFIX -- add this when moved into re_move
...
void ml_tree_re_move(tree* t, node*p, node** q, boolean doinit)
{
  ...
  /* EWFIX.TREECHECK
  printf("EWFIX.REMOVE start of %p \n",p);
  t->tree_print_f(t);
  assert(t->tree_good_f(t));
  */
  ...
  /* EWFIX.TREECHECK
  printf("EWFIX.REMOVE end  of %p %p\n",p,*q);
  t->tree_print_f(t);
  assert(t->tree_good_f(t));
  */
...
static boolean ml_tree_try_insert_thorough(tree* t, node* p, node* q, 
    ...
    boolean* multf)
{
  ...
  if ( multf )
    *multf = false;
  if ( thorough)
    succeeded = ml_tree_try_insert_thorough(t, p, q, qwherein, bestyet, bestree,
        priortree);
  else
 ...
 */
static boolean ml_tree_try_insert_notthorough(tree *t, node *p, node *q,
    node** qwherein, double* bestyet)
{
  ...
  t->save_traverses(t, p, q);
      ...
      /* EWFIX.TREECHECK
      printf("EWFIX -- before insert of %p %p\n",p,q);
      t->tree_print_f(t);
      assert(t->tree_good_f(t));
      */
  ...
  t->insert_(t, p, q, false, false);
      ...
      /* EWFIX.TREECHECK
      printf("EWFIX -- after  insert of %p %p\n",p,q);
      t->tree_print_f(t);
      assert(t->tree_good_f(t));
      */
  ...
  like = t->evaluate(t, p, false);
      ...
      /* EWFIX.TREECHECK
      printf("EWFIX -- after evaluat of %p %p\n",p,q);
      t->tree_print_f(t);
      assert(t->tree_good_f(t));
      */
  ...
  t->re_move(t, p, &whereRemoved, false);   /*EWFIX -- check doinit value*/

      /* EWFIX.TREECHECK
      printf("EWFIX -- after re_move of %p %p\n",p,q);
      t->tree_print_f(t);
      assert(t->tree_good_f(t));
      */
  ...
  t->restore_traverses(t, p, q);
      ...
      /* EWFIX.TREECHECK
      printf("EWFIX -- after travers of %p %p\n",p,q);
      t->tree_print_f(t);
      assert(t->tree_good_f(t));
      */
  ...
  t->evaluate(t, q, 0);
      ...
      /* EWFIX.TREECHECK
      printf("EWFIX -- after re-eval of %p %p\n",p,q);
      t->tree_print_f(t);
      assert(t->tree_good_f(t));
      */
...
void mlk_tree_insert_(tree *t, node *newtip, node *below, boolean dummy, 
    boolean dummy2)
{
  ...
  if (newfork->back != NULL) {
    ...
    if (((ml_node*)p)->node.tyme > ((ml_node*)newfork->back)->node.tyme)
      set_tyme(newfork,
          (((ml_node*)p)->node.tyme + ((ml_node*)newfork->back)->node.tyme) / 2.0); 
    else 
    ...
    do {
      ...
      if (!done)
        done = (((ml_node*)t->nodep[p->back->index - 1])->node.tyme 
            < ((ml_node*)p)->node.tyme);
      if (!done) {
...
void mlk_tree_re_move(tree* t, node *item, node** where, boolean doinit)
{
  /* removes nodes item and its ancestor, fork, from the tree.
    ...
    returns pointers to the deleted nodes, item and fork, and records
    ...
    boolean progress, tree* priortree, tree* bestree, inittravtree_t inittravtree)
{
  ...
  if (reusertree) { 
    ...
    if (global)
      ...
      curtree->locrearrange(curtree, curtree->root->back, smoothit, priortree,
        bestree);
    polishing = true;
    smoothit = true;
    curtree->smoothall(curtree, curtree->root);
