...
node* generic_tree_get_fork(tree* t)
{
  ...
  if ( Slist_isempty(t->free_fork_nodes) ) {   /* make a ring of 3 nodes */
    ...
    retval = Slist_pop(t->free_forks);   /* pop it off the stack if there */
  }
...
{ /* release the fork attached to a removed node, and put it back on the forks list */
  node *p;
  long sibs;
...
{ /* put a fork node circle onto the garbage list */
  n->reinit(n);
