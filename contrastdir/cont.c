...
#include "cont.h"
#include "ml.h"
/* version 3.6. (c) Copyright 1999-2011 by the University of Washington.
...
{ /* make a new node of type cont_node_type */
  ...
  n = Malloc(sizeof(cont_node_type));
...
void alloctree(pointarray *treenode, long nonodes)
{
  ...
  *treenode = (pointarray)Malloc(nonodes*sizeof(node *));
  ...
  for (i = spp; i < nonodes; i++) {
    /* node allocation handled in treeread which calls
       initcontrastnode or initthreshmlnode */
    (*treenode)[i] = NULL;
  }
} /* alloctree */
