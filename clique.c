...
void clique_run(void);
void clique(char *, char *, char *, char *, char *, char *, char *, char *, int, int, int, int, int, int, int, int, int, int, int, int, int, int, int);
          ...
          case 'C':
            ...
            if (Clmin)
            {
              ...
              do {
                ...
                phyFillScreenColor();
                if(scanf("%ld%*[^\n]", &Cliqmin)) {} // Read number and scan to EOL.
                (void)getchar();
...
void clique_setuptree(void)
{
  ...
  long i;
  //printf("in clique_setuptree spp: %li\n", spp); // JRMdebug
  ...
  //printf("size of node: %li size of treenode: %li\n", sizeof(node), sizeof(treenode));// JRMdebug
  for (i = 0; i < spp; i++)
  {
    //printf("in loop i: %li\n", i);// JRMdebug
    if(treenode[i] != NULL)
...
void doinit(void)
{
  ...
  //printf("%li species, %li  characters %li nonodes\n", spp, chars, nonodes); // JRMdebug
  if (!javarun)
  {
    getoptions();
  }
...
void Gen1(long i, long CurSize, boolean *aChars, boolean *Candidates, boolean *Excluded)
{
  ...
  if (Factors)
  {
    ...
    for (i = 1; i <= ActualChars; i++)
    {
      if (aChars[i - 1])
      {
        ...
        j++;
        newline(outfile, j, (long)((FormWide - 22) / 3), (long)nmlngth + 1);
      }
    }
    fprintf(outfile, ")\n");
  }
  ...
  for (i = 1; i <= chars; i++)
  {
    if (aChars[ActChar[i - 1] - 1])
    {
      ...
      if (Factors)
        ...
        newline(outfile, j, (long)((FormWide - 15) / 3), (long)nmlngth + 1);
    }
  }
  fprintf(outfile, ")\n\n");
}  /* PrintClique */
...
void recontraverse(node **p, long *st, long n, long MaxChars)
{
  ...
  while (!zero2)
  {
    ...
    ((clique_node*)q->back)->maxpos = maxpos;
    q->back->back = q;
    for (j = 0; j < setsz; j++)
      st2[j] &= ~tempset[j];
...
void clique_coordinates(node *p, long *tipy, long MaxChars)
{
  ...
  while (q->next != p)
    q = q->next;
  last = q->back;
  p->xcoord = (MaxChars - ((clique_node*)p)->maxpos) * 3 - 2;
  if (p == root)
    p->xcoord += 2;
  ...
  do {
    ...
    if ((long)q->ycoord == i && !done)
    {
      if (!q->tip)
      {
        ...
        plus = true;
        j = 1;
        for (k = 1; k <= (((clique_node*)q)->maxpos); k++)
        {
          same = true;
          for (l = 0; l < setsz; l++)
            if (grouping[k - 1][l] != grouping[((clique_node*)q)->maxpos - 1][l])
              same = false;
          if (same)
          {
            poslist[j - 1] = k;
    ...
    else if (!p->tip && (long)last->ycoord > i && (long)first->ycoord < i && (i != (long)p->ycoord || p == root))
    {
      putc('!', outfile);
      for (j = 1; j < n; j++)
        putc(' ', outfile);
...
void DoAll(boolean *Chars_, boolean *Processed, boolean *Rarer_, long tcount)
{
  ...
  //printf("in DoAll calling clique_setuptree\n");// JRMdebug
  clique_printree();
  if (trout)
  {
    col = 0;
...
void Gen2(long i, long CurSize, boolean *aChars, boolean *Candidates, boolean *Excluded)
{
  /* finds largest size cliques and prints them out */
  long CurSize2, j, k, Actual, Possible;
  boolean Futile;
...
node * clique_node_new(node_type type, long index)
{
  node *n = Malloc(sizeof(clique_node));
...
void clique_run(void)
{
  for (ith = 1; ith <= msets; ith++)
  {
    ...
    if (progress)
    {
      ...
      sprintf(progbuf, " written on file \"%s\".\n\n", outtreename);
      print_progress(progbuf);
    }
  }
  FClose(infile);
...
void clique(
  ...
  int noCliqueSize,
  int intCliqmin,
  int multipleDataSets,
  int multipleWeightSets,
  int useOutGroupRoot,
  int intoutgrno,
  int printRawData,
  int progressBar,
  ...
  int outputTree)
{
  ...
  if (multipleDataSets != 0)
    mulsets = true;
  else
    mulsets = false;

  if (multipleWeightSets != 0)
    justwts = true;
  else
    justwts = false;

  if (useOutGroupRoot != 0)
    outgropt = true;
  else
    outgropt = false;
