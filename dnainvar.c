...
void dnainvarrun(void);
void dnainvar(char * infilename, char * outfilename, char * outfileopt, char * weightsfilename,
              int SitesWeighted, int AnalyzeMult, int NumMult, int MultDataSet, int InputSeq,
              int PrintData, int DotDiff, int PrintInd, int PrintInvar, int PrintPats);
/* function prototypes */
#endif

Char infilename[FNMLNGTH], outfilename[FNMLNGTH], weightfilename[FNMLNGTH];
long sites, msets, ith, nonodes = 0;
...
void prntpatterns(void)
{
  ...
  for (i = 0; i < endsite; i++) {
    fprintf(outfile, "     ");
    for (j = 0; j < spp; j++)
      putc(inputSequences[j][alias[i] - 1], outfile);
    if (prntpat)
...
void makesymmetries(void)
{
  ...
  for (i = 0; i < endsite; i++) {
    drop = false;
    for (j = 0; j < spp; j++) {
      ch = inputSequences[j][alias[i] - 1];
      drop = (drop ||
              (ch != 'A' && ch != 'C' && ch != 'G' && ch != 'T' && ch != 'U'));
    }
    ch1 = inputSequences[0][alias[i] - 1];
    if (!drop) {
      usedz = false;
      zchar = ' ';
      for (j = 2; j <= spp; j++) {
        ch = inputSequences[j - 1][alias[i] - 1];
        if (ch == ch1)
          t[j - 2] = xx;
...
void tabulate(long mm, long nn, long pp, long qq, double *mr, double *nr, double *pr, double *qr)
{
...
void dnainvarrun(void)
{
  ...
  */
  ...
  if (progress) {
    sprintf(progbuf, "\n");
    print_progress(progbuf);
    sprintf(progbuf, "Output written to output file \"%s\"\n", outfilename);
    print_progress(progbuf);
    sprintf(progbuf, "\n");
    print_progress(progbuf);
  }
...
void dnainvar(
  ...
  int PrintPats)
{
  (void)MultDataSet;                    // RSGnote: Parameter never used.

  //printf("Hello from DnaInvar!\n"); // JRMdebug
  //fflush(stdout);
